...
{
    public partial class EditSliderImageDialog
    {
        protected bool busy;
        protected EditSliderImageDtoValidator _imageValidator;
        protected ServerValidation _serverValidator;
        [CascadingParameter]
        MudDialogInstance DialogInstance { get; set; }

        [Parameter]
        public EditSliderImageDto SliderImage { get; set; }

        [Parameter]
        public EventCallback SliderImageEdited { get; set; }
        ...
        protected async Task EditImageAsync()
        {
            busy = true;
            try
            {
                await SliderImageService.EditSliderImageAsync(SliderImage);
                if (SliderImageEdited.HasDelegate)
                    await SliderImageEdited.InvokeAsync();
