...
{
    public class StartAboutUsImagesResizingJob
    {
        ...
        public class Handler : IRequestHandler<Request, Unit>
        {
            ...
            public async Task<Unit> Handle(Request request, CancellationToken cancellationToken)
            {
                var currentJob = await _context.JobDetailsRepository
                    .Where(j => j.JobTypeId == (int)JobType.ResizingAboutUsImages &&
                ...
                var jobId = BackgroundJob.Schedule(() => _realTimeNotifierImagesManipulatorService.ResizeAboutUsImagesAsync(null, cancellationToken), TimeSpan.FromSeconds(10));
                await _context.JobDetailsRepository.AddAsync(new Domain.Jobs.JobDetail
                {
                    JobId = jobId,
                    Status = JobStatus.Pending,
                    Type = JobType.ResizingAboutUsImages,
                }, cancellationToken);

                await _context.SaveChangesAsync(_loggedInUserProvider.UserId, cancellationToken);

                await _hubContext.Clients.Group(SignalRGroups.AboutUsImagesResizeProgress).SendAsync(ServerMessages.ResizingAboutUsImagesJobAdded, cancellationToken);
                return Unit.Value;
            }
        }
    }
